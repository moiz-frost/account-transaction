class CookieController < ApplicationController
  ...
  def login
    render(
      json: { error: 'Password is not present' }
    ), status: :bad_request and return if params[:password].blank?

    render(
      json: { error: 'Please enter an email or phone no' }
    ), status: :bad_request and return if params[:email].blank? && params[:phone_number].blank?
    ...
    account = if email.present?
      ...
      Account.find_by_phone_number(phone_number)
    end
